...
const drawElement = (_renderObject, _index, _layersLen) => {
	...
	text.only
		? addText(`${_renderObject.layer.name}${text.spacer}${_renderObject.layer.selectedElement.name}`, text.xGap, text.yGap * (_index + 1), text.size)
...
const createDna = (_layers) => {
	...
	_layers.forEach((layer) => {
		...
		for (var i = 0; i < layer.elements.length; i++) {
			...
			if (random < 0) {
				return randNum.push(`${layer.elements[i].id}:${layer.elements[i].filename}${layer.bypassDNA ? "?bypassDNA=true" : ""}`);
			}
		}
	});
...
const startCreating = async () => {
	...
	for (let i = 0; i <= layerConfigurations[layerConfigurations.length - 1].growEditionSizeTo; i++) {
